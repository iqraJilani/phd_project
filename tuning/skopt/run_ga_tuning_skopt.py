...
# arg_parser.add_argument('--prob_crossover')
# arg_parser.add_argument('--prob_mutation')
...
pop_size = 400
num_gen = 200
# p_c = args.prob_crossover
# p_m = args.prob_mutation
p_c = 0.2
p_m = 0.9
...
def evaluate_ga(params):
    ...
    globals()['p_m'], globals()['p_c'] = params
    tuning_pm_list.append(params[0])
    tuning_pc_list.append(params[1])
    ...
    best_fv_avg = (np.sum(best_fv_all_days))/n_days
    log = { 'tuning_fitness': best_fv_avg, 'tuning_pm': params[0], 'tuning_pc': params[1] }

    #wb.log({'eval_call_fitness': log, 'eval_call_counter': globals()['current_call']})
    wb.log(log)
    
    best_fv_avg = -1 * best_fv_avg
    return best_fv_avg
...
space = [Real(0.6, 0.9, name='prob_mutation'), 
         Real(0.4, 0.7, name='prob_crossover')]

n_calls_func = 10
wb.config.update({"no of function calls": n_calls_func})

tuning_result = gp_minimize(evaluate_ga, space, n_calls= n_calls_func , random_state=42)
best_params = tuning_result.x
best_fitness = tuning_result.fun
skopt_models = tuning_result.models
skopt_model = skopt_models[-1]
...
wb.config.update({"best_fv_average": best_fitness}, allow_val_change=True)

with open('/nvme/jilanii/parameter_tuning/output_pickle/skopt_models.pickle', 'wb') as f:
    pickle.dump(skopt_models, f)

with open('/nvme/jilanii/parameter_tuning/output_pickle/skopt_model.pickle', 'wb') as f:
    pickle.dump(skopt_model, f)

with open('/nvme/jilanii/parameter_tuning/output_pickle/tuning_pc.pickle', 'wb') as f:
    pickle.dump(tuning_pc_list, f)

with open('/nvme/jilanii/parameter_tuning/output_pickle/tuning_pm.pickle', 'wb') as f:
    pickle.dump(tuning_pm_list, f)


plt.figure(figsize=(12, 6))

plt.subplot(1, 2, 1)
pc_unique, pc_counts = np.unique(tuning_pc_list, return_counts=True)
plt.bar(pc_unique, pc_counts, edgecolor='black')
plt.xlabel("probability")
plt.ylabel("frequency distribution")
plt.title("Distribution of Probability of Crossover")

plt.subplot(1, 2, 2)
pm_unique, pm_counts = np.unique(tuning_pm_list, return_counts=True)
plt.bar(pm_unique, pm_counts, edgecolor='black')
plt.xlabel("probability")
plt.ylabel("frequency distribution")
plt.title("Distribution of Probability of Mutation")

plt.savefig("probability_distribution.jpeg", format='jpeg')
plt.close()
