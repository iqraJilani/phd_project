...
run_name = "tuning_smac_scenario2"
...
data = pd.read_excel(dataset_path, 'scenario_2_training') # energy usage data
...
smac_optimizer = HyperparameterOptimizationFacade(scenario, target_function=evaluate_ga, overwrite=True) 
incumbent = smac_optimizer.optimize() 

final_pm, final_pc = incumbent['p_m'], incumbent['p_c'] 
best_fitness = smac_optimizer.runhistory.get_cost(incumbent)
...
smac_model =  smac_optimizer._intensifier._config_selector._model


with open('/nvme/jilanii/parameter_tuning/output_pickle/smac_model.pickle', 'wb') as f:
    pickle.dump(smac_model, f)

with open('/nvme/jilanii/parameter_tuning/output_pickle/tuning_pc_smac.pickle', 'wb') as f:
    pickle.dump(tuning_pc_list, f)

with open('/nvme/jilanii/parameter_tuning/output_pickle/tuning_pm_smac.pickle', 'wb') as f:
